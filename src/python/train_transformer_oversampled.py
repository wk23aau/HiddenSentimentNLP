...
def load_and_prepare_data(file_path, text_col="reviewText_clean", sentiment_col="sentiment"):
    """
    Loads the CSV file, fills missing text values, and ensures a sentiment column exists.
    ...
    if sentiment_col not in df.columns:
        if "reviewRating" in df.columns:
            ...
            print(f"Error: Sentiment column '{sentiment_col}' not found and no reviewRating or rating column to create it.")
            exit(1)
    return df

def train_transformer(train_file, model_name="bert-base-uncased", output_dir="models/bert-sentiment", epochs=2, batch_size=8):
    """
    Fine-tunes a transformer model on the provided training data.
    """
    ...
    # Load the pretrained tokenizer and tokenize the review texts
    ...
    # Create custom PyTorch datasets using the 'sentiment' column
    ...
    # Load the pretrained model for sequence classification (binary classification)
...
if __name__ == "__main__":
    ...
    # Prompt the user to select the preprocessed, oversampled CSV file.
    file_path = filedialog.askopenfilename(
        title="Select Preprocessed Oversampled CSV File",
    ...
    MODEL_OUTPUT_DIR = "models/bert-sentiment"
